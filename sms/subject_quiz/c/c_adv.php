...
$existingScoreQuery = "SELECT c_adv FROM scores WHERE email = '$email'";
...
if ($existingScoreResult->num_rows > 0) {
    ...
    $existingScore = $row['c_adv'];

    // Update the score only if the new score is higher
    if ($userScore > $existingScore) {
        $updateScoreQuery = "UPDATE scores SET c_adv = $userScore WHERE email = '$email'";
    ...
    $insertScoreQuery = "INSERT INTO scores (email, c_adv) VALUES ('$email', '$userScore')";
