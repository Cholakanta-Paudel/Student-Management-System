...
$existingScoreQuery = "SELECT ex_inter FROM scores WHERE email = '$email'";
...
if ($existingScoreResult->num_rows > 0) {
    ...
    $existingScore = $row['ex_inter'];

    // Update the score only if the new score is higher
    if ($userScore > $existingScore) {
        $updateScoreQuery = "UPDATE scores SET ex_inter = $userScore WHERE email = '$email'";
    ...
    $insertScoreQuery = "INSERT INTO scores (email, ex_inter) VALUES ('$email', $userScore)";
