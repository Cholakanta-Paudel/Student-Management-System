...
$existingScoreQuery = "SELECT c_final FROM scores WHERE email = '$email'";
...
if ($existingScoreResult->num_rows > 0) {
    ...
    $existingScore = $row['c_final'];

    // Update the score only if the new score is higher
    if ($userScore > $existingScore) {
        $updateScoreQuery = "UPDATE scores SET c_final = $userScore WHERE email = '$email'";
    ...
    $insertScoreQuery = "INSERT INTO scores (email, c_final) VALUES ('$email', '$userScore')";
