...
$existingScoreQuery = "SELECT se_adv FROM scores WHERE email = '$email'";
...
if ($existingScoreResult->num_rows > 0) {
    ...
    $existingScore = $row['se_adv'];

    // Update the score only if the new score is higher
    if ($userScore > $existingScore) {
        $updateScoreQuery = "UPDATE scores SET se_adv = $userScore WHERE email = '$email'";
    ...
    $insertScoreQuery = "INSERT INTO scores (email, se_adv) VALUES ('$email', $userScore)";
