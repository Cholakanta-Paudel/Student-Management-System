...
$existingScoreQuery = "SELECT se_final FROM scores WHERE email = '$email'";
...
if ($existingScoreResult->num_rows > 0) {
    ...
    $existingScore = $row['se_final'];

    // Update the score only if the new score is higher
    if ($userScore > $existingScore) {
        $updateScoreQuery = "UPDATE scores SET se_final = $userScore WHERE email = '$email'";
    ...
    $insertScoreQuery = "INSERT INTO scores (email, se_final) VALUES ('$email', $userScore)";
